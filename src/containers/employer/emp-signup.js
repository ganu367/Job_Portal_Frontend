...
function EmpSignup() {
    ...
    const handleSignup = async (event) => {
        ...
        const data = {
            username: username,
            password: password,
            company_name: name.toString(),
            country_code: country.toString(),
            mobile_number: mobile
        };
        try {
            setLoading(true);
            const response = await axios.post("/auth/register/new-employer",
            data,
            {
    ...
    return (
        <>
        <Form>
            <Form.Title>EMPLOYER SIGN UP</Form.Title>
            <Form.Line />
            {error &&
            ...
            <Form.ToggleGroup>
                <Form.Toggle onClick={() => {
                    navigate("/candidate/signup", {state: { from: location }});
                    resetInputFields();
                }}>
                    Signup as candidate
                </Form.Toggle>
            ...
            <Form.Base onSubmit={handleSignup} >
                ...
                <Form.InputColumn>
                    <Form.InputContainer refPointer={countryRef} width={"30%"}>
                        ...
                        <Dropdown width={"500%"} dropdown={countryDD} flexDirection="column">
            ...
            <Form.Text>
                Already have an account? <Form.Link to="/employer/signin">Sign in now.</Form.Link>
            </Form.Text>
        </Form>
        </>
    );
