...
import { CandUpdateCommentModal } from "../../containers";
import { useAxiosPrivate, useAlert, useDateFormat, useLoading } from "../../hooks";
import { FaTimes } from "react-icons/fa";
import { FiChevronDown, FiChevronUp } from "react-icons/fi";
import { jobTenureList, jobTypeList, jobModeList, jobApplicationType } from "../../constants";
...
export default function CandJobApplications({children, statusType}) {
    ...
    const [viewComment, setViewComment] = useState({comment: "", jobID: ""});
    ...
    //     axiosPrivate
    //     .get(`/api/candidate/job/view-${statusType}-jobs/`+currentPage)
    ...
    const viewStatusTypeJobs = () => {
        axiosPrivate
        .post(`/search/candidate/filtered-${statusType}-jobs/`+currentPage, {location: searchLoc.toString(), skill: searchSkills.toString(), title: searchTitles.toString(), qualification: searchQuali.toString(), function: searchFunc.toString(), type: searchType, mode: searchMode, tenure: searchTenure, exp: searchExp, sal: searchSal},
        {headers: {
    ...
    const handleAction = () => {
        if(action === "Apply") {
            navigate(`/candidate/job/${selected[0]}/assignment`, {state: { from: location }});
            setAction("");
        }
            ...
            setLoading(true);
            axiosPrivate
            .put("/api/candidate/job/reapply-job/"+selected[0])
            .then(function (response) {
                setLoading(false);
                setAlert({msg: `Success: ${response?.data}`, type: "success"});
                // viewAppliedJobs();
    ...
    return (
        ...
        <Card rowDirection noPadding>
            <Card.RestContainer>
                <PostCard>
                    <Card.Title>{jobApplicationType.find((app) => {return app.type === statusType}).title}</Card.Title>
                    <PostCard.Line />
                    <PostCard.ButtonGroup flexStart marginBottom>
                        {(statusType === "") &&
                            <Button small disabled={selected.length === 0} onClick={() => setAction("Apply")}>Apply</Button>
                        }
                        {(statusType === "withdrawn") &&
                            <Button small disabled={selected.length === 0} onClick={() => setAction("Reapply")}>Re-apply</Button>
                        }
                        {(statusType === "applied") &&
                            <Button danger small disabled={selected.length === 0} onClick={() => setAction("Withdraw")}>Withdraw</Button>
                        }
                                ...
                                <PostCard.Post key={job.id}>
                                    ...
                                    <PostCard.Column fullHeight width="100%">
                                            ...
                                            <PostCard.Group>
                                                ...
                                                {job.total_interviews ?
                                                    <PostCard.Column width="fit-content" center marginRight>
                                                        ...
                                                        <PostCard.Text>{job.total_interview_score ? job.total_interview_score : "0"} / {(job.total_interviews * 10)}</PostCard.Text>
                                                    </PostCard.Column>
                                                    : null
                                                }
                                                <PostCard.Column width="fit-content" center marginRight>
                                                    <PostCard.Label paddingBottom>Eval score:</PostCard.Label>
                                                    <PostCard.Text>{job.evaluation_score} / 10</PostCard.Text>
                                                </PostCard.Column>
                                                {job.started_date &&
                                        ...
                                        <PostCard.Row center flexEnd noMargin>
                                            {(job.candidate_comment !== null && job.candidate_comment !== "") &&
                                                <PostCard.Row center noMargin>
                                                    <Button small onClick={() => setViewComment({comment: job.candidate_comment, jobID: job.id})}>View comment</Button>
                                                </PostCard.Row>
                                            }
                                            {(job.candidate_comment === null || job.candidate_comment === "") &&
                                                <PostCard.Row center noMargin>
                                                    <Button small onClick={() => setViewComment({comment: "", jobID: job.id})}>Add comment</Button>
                                                </PostCard.Row>
                                            }
                                            {/* <PostCard.Column width="100%"> */}
                                            <PostCard.Row center flexEnd noMargin>
                                                ...
                                                <Button small onClick={() => navigate(`/candidate/job/${job.id}`, {state: {from: location, status: "applied"}})}>View Details</Button>
                                                <Button noMargin iconPadding small onClick={() => navigate(`/candidate/jobs/chat`, {state: {from: location, jobID: job.id}})}><Button.Icon alone><BiMessageDetail /></Button.Icon></Button>
        ...
        {(viewCommentModal) ?
            <CandUpdateCommentModal viewComment={viewComment} setViewComment={setViewComment} setCommentUpdated={setCommentUpdated} setViewCommentModal={setViewCommentModal} />
        : null}
        </>
        // </Container>
    );
